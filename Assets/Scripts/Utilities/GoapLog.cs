...
{
    public enum Level { Off = 0, Error = 1, Warn = 2, Info = 3, Verbose = 4 }
    ...
    public static void Verbose(string key, string msg, Object ctx = null, float minInterval = 0.2f)
    ...
    {
        if (Current < lvl || Current == Level.Off) return;

        float now = Time.time;
        if (minInterval > 0f)
        {
            if (_nextTime.TryGetValue(key, out float next) && now < next) return;
            _nextTime[key] = now + minInterval;
        }

        switch (lvl)
        {
            case Level.Error: Debug.LogError(msg, ctx); break;
            case Level.Warn: Debug.LogWarning(msg, ctx); break;
            default: Debug.Log(msg, ctx); break;
        }
    }
}
